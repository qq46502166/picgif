...
abstract class Post extends Model
{
    ...
    public function search($where=[],$limit='0,10',$order='id desc',$single=false,$select=''){
        if(!$select)
            $select=$this->table.'.*,user.username as username ,user.avatar as avatar,category.name as category_name,category.slug as category_slug,category.thumb as category_thumb';
        else
