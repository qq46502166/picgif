		...
		<div class="content-detail">
			<div class="content-button">
				<div class="content-top">
					<a href="<?=url('admin/portal/post_add')?>?type=<?=$type?>" class="pure-button btn-custom">添加<?=$pindao[$type]?></a>
				</div>
			</div>
						...
						<div class="pure-u-1-2">
							<div class="dh-dropdown">
								...
								<ul class="dh-dropdown-content delete-dropdown-content">
									<li class="dh-dropdown-item delete-all"><a href="<?=url('admin/portal/post_del')?>">删除</a></li>
								</ul>
							</div>
						</div>
					...
					<table class="full pure-table pure-table-bordered">
						...
						<?php foreach($posts as $post):?>
							<tr>
								...
								<td><?=$post['id']?></td>
								<td><a href="<?=url('admin/portal/post_edit','id='.$post['id'])?>"><?=$post['title']?><i class="iconfont icon-"></td>
								<td><?=$post['category_name']?></td>
								<td><?=$post['status']?></td>
								<td><?=date('Y-m-d H:i',$post['create_time'])?></td>
								<td>
									<a class="pure-button btn-success btn-xs edit" href="<?=url('admin/portal/post_edit')?>?type=<?=$type?>&id=<?=$post['id']?>" target="_bank">编辑</a>
									<a class="pure-button btn-warning btn-xs delete" href="<?=url('admin/portal/post_del')?>?type=<?=$type?>&id=<?=$post['id']?>">删除</a>
								</td>
							</tr>
						<?php endforeach;?>
						</tbody>
...
$(function(){
	//侧栏
	$("#admin-portal-post-<?=$type?>").addClass('active-this');
	ajaxLink(".delete");
