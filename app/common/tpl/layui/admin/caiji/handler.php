        ...
        <div class="layui-tab-content">
            <div class="layui-tab-item layui-show">
                <div class="layui-form">
                    ...
                    <?php if($data): ?>
                    <form method="post" action="<?=url('api/caijiAdmin/action_multi')?>" name="form2" id="form2">
                        ...
                        <?php /*dump($data);*/ $thumbFormat=getThumbFormat();$model=app('app\admin\model\Tag');foreach ($data as $item): $tags_name=$model->getName($item['id'],'bbs_normal');?>
                            <tr id="c<?=$item['id']?>">
                                ...
                                <td style="text-align: center;">
                                    ...
                                    <i class="layui-icon" style="font-size: 20px;">
                                        <a title="设为垃圾" id="deletea_<?=$item['id']?>" href="javascript:;" ac="<?=url('api/caijiAdmin/set_laji')?>?table=<?=$get['table']?>&id=<?=$item['id']?>">
                                            &#xe640;
                                        </a>
                                    </i>
                                </td>
            ...
            $("a[id^='add-tag-").on('click', function(){
                ...
                layer.open({
                    ...
                    yes:function (index, layero) {
                        ...
                        fly.json("<?=url('api/caijiAdmin/tag_edit')?>",{id:id,title:newTitle,content:newContent,tag:tags,table:"<?=$get['table']?>"},function (res) {
                            layer.msg(res.msg);
                    ...
                    btn2: function(index){
                        layer.confirm('真的要设为垃圾？',function (index2) {
                            fly.json("<?=url('api/caijiAdmin/set_laji')?>",{id:id,table:"<?=$get['table']?>"},function (res) {
                                layer.msg(res.msg);
                                _this.parent().parent().remove();
                                layer.close(index2);
                                layer.close(index);
