...
var asideMenu=function(){
	var _this=$(".dh-menu");
	//先搜索active-this
	var select=_this.find('.active-this');
	if(select.length<1)
		select=_this.find("li[router='"+router.module+'/'+router.ctrl+'/'+router.action+"']");
	if(select.length <1 ){
		...
		//select.addClass("dh-menu-select").parents(".dh-menu-has-children").show();
	}
...
function var_dump (theVar,notToSource) {
	if(notToSource)
